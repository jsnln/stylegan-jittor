...
def init_linear(linear):
    jt.init.xavier_gauss_(linear.weight)
    linear.bias.data.zero_()

def init_conv(conv, glu=True):
    jt.init.kaiming_normal_(conv.weight)
...
def equal_lr(module, name='weight'):
    ...
    setattr(module, name + '_orig', weight.clone())
...
class FusedUpsample(jt.Module):
    def __init__(self, in_channel, out_channel, kernel_size, padding=0):
        ...
        ### NOTE 'parameter' not needed in jittor
        # self.weight = jt.nn.Parameter(weight)
        # self.bias = jt.nn.Parameter(bias)
        self.weight = weight
        self.bias = bias
        self.pad = padding
...
class Blur(jt.Module):
    def __init__(self, channel):
        ...
        ### NOTE 'buffer' are not learnable in pytorch
        ### NOTE in jittor they should presumably be named starting with an underscore``
    ...
    def execute(self, input):
        ### TODO use the actual blur function
...
class AdaptiveInstanceNorm(jt.Module):
    def __init__(self, in_channel, style_dim):
        ...
        self.norm = jt.nn.InstanceNorm2d(in_channel)
...
class Discriminator(jt.Module):
    ...
    def execute(self, input, step=0, alpha=-1):
        for i in range(step, -1, -1):
            ...
            if i == 0:
                ### NOTE modified: sqrt(var)
                # out_std = jt.sqrt(out.var(0, unbiased=False) + 1e-8)
                ### NOTE below is jittor
                def jt_std(x, dim):
                    matsize = x.size(dim)

                    out_tmp = (x - x.mean(dim, keepdims=True)).sqr().sum(dim)
                    out_tmp = out_tmp / (matsize - 1)
                    out_tmp = out_tmp.maximum(1e-8).sqrt()
                    return out_tmp
                
                out_std = jt_std(out, 0)
